...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    /* First get the number of bytes in the file */
    ...
    /* Set buffer equal to size of file */
    buffer = (char*)calloc(numbytes, sizeof(char));

    /* check for memory erro */
    if (buffer == NULL)
    {
      return false;
    }
    ...
    /* Assign buffer and length pointers to content and length */
    *content = buffer;
    *length = numbytes;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // Create temporary string to hold request line
    ...
    /* Get abs_path and query */

    // First, check to see if there are query parameters
    if (strchr(line, '?') != NULL)
    {
      // If so, parse and store them and abs_path
      strcpy(abs_path, strtok(NULL, "?"));
    ...
    /* Make sure abs_path begins with a / */
    if (abs_path[0] != '/')
    {
      error(501);
      return false;
    ...
    /* Check to make sure abs_path does not contain a " */
    for (int i = 0, length = strlen(abs_path); i < length; i++)
    {
      if (abs_path[i] == '"')
      {
        error(400);
    ...
    /* Check to make sure version is HTTP/1.1 */
